    ...
    <div class="container-fluid px-2 px-lg-4 mb-4">
        ...
        <header class="mb-4 px-4 py-3 bg-white border rounded-3 shadow-sm d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div>
                ...
                <small class="text-muted d-block mt-1">Gestão centralizada de clientes, responsáveis legais e contatos principais.</small>
            </div>
            ...
            <div class="card-body p-4">
                            ...
                            <tbody>
                                <?php foreach ($empresas as $emp): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <div class="fw-semibold text-dark"><?= htmlspecialchars($emp['nome_fantasia'] ?? $emp['nome'] ?? $emp['razao_social'] ?? 'Sem nome') ?></div>
                                            <?php if (!empty($emp['razao_social']) && ($emp['nome_fantasia'] ?? '') !== $emp['razao_social']): ?>
                                                <small class="text-muted"><?= htmlspecialchars($emp['razao_social']) ?></small>
                                            <?php endif; ?>
                                        </td>
                                        <td class="font-monospace text-muted fw-bold"><?= !empty($emp['cnpj']) ? htmlspecialchars($emp['cnpj']) : '-' ?></td>
                ...
                <?php else: ?>
                    <div class="text-center py-5 text-muted">
                        ...
                        <p class="small text-muted mb-3">Clique no botão abaixo para adicionar a primeira empresa.</p>
