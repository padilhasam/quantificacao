    ...
    <div class="container-fluid px-2 px-lg-4 mb-4">
        ...
        <header class="page-header-riscos mb-4 px-4 py-3 bg-white border rounded-3 shadow-sm d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div>
                ...
                <small class="text-muted d-block mt-1">
                    Atualize as informações técnicas do risco cadastrado.
                </small>
            </div>

            <a href="<?= BASE_URL ?>/riscos/<?= htmlspecialchars($slug ?? '') ?>" class="btn btn-outline-secondary btn-sm rounded-pill px-3 fw-medium">
        ...
        <div class="card border-0 shadow-sm rounded-3">
            <div class="card-body p-4">
                ...
                <form action="<?= BASE_URL ?>/riscos/atualizar/<?= (int)$risco['id'] ?>" method="POST" class="needs-validation" novalidate>

                    <input type="hidden" name="categoria" value="<?= htmlspecialchars($risco['categoria'] ?? $categoria ?? '') ?>">
                    ...
                    <div class="border rounded-3 p-3 mb-4 bg-light-subtle">
                        ...
                        <div class="row g-3">
                            <div class="col-12 col-md-3">
                                ...
                                <input type="text" name="codigo" class="form-control bg-light border-dark-subtle text-uppercase" value="<?= htmlspecialchars($risco['codigo'] ?? '') ?>" readonly>
                            </div>

                            <div class="col-12 col-md-3">
                                <label class="form-label fw-semibold text-secondary small">Código Externo</label>
                                <input type="text" name="codigo_externo" class="form-control bg-light border-dark-subtle text-uppercase" value="<?= htmlspecialchars($risco['codigo_externo'] ?? '') ?>" readonly>
                            </div>

                            <div class="col-12 col-md-6">
                                <label for="nome" class="form-label fw-semibold text-secondary small">Nome do Risco *</label>
                                <input type="text" name="nome" id="nome" class="form-control border-dark-subtle" maxlength="150" required value="<?= htmlspecialchars($risco['nome'] ?? '') ?>">
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <select name="tipo_avaliacao" id="tipo_avaliacao" class="form-select border-dark-subtle">
                                    <?php foreach (['Qualitativo','Quantitativo','Qualitativo/Quantitativo'] as $opcao): ?>
                                        <option value="<?= $opcao ?>" <?= (($risco['tipo_avaliacao'] ?? '') === $opcao) ? 'selected' : '' ?>>
                                            <?= $opcao ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <input type="text" name="unidade_medida" id="unidade_medida" class="form-control border-dark-subtle" value="<?= htmlspecialchars($risco['unidade_medida'] ?? '') ?>">
                            </div>
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <div class="form-check form-switch d-flex align-items-center gap-3 ps-0 mt-2">
                                    <input class="form-check-input m-0 border-dark-subtle" type="checkbox" id="exige_quantificacao" name="exige_quantificacao" value="1" <?= !empty($risco['exige_quantificacao']) ? 'checked' : '' ?>>
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="descricao" id="descricao" rows="3" class="form-control border-dark-subtle"><?= htmlspecialchars($risco['descricao'] ?? '') ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border rounded-3 p-3 mb-4 bg-light-subtle">
                        ...
                        <div class="row g-3">
                            <div class="col-12 col-md-4">
                                ...
                                <input type="text" name="limite_nr15" id="limite_nr15" class="form-control border-dark-subtle" value="<?= htmlspecialchars($risco['limite_nr15'] ?? '') ?>">
                            </div>

                            <div class="col-12 col-md-4">
                                <label for="limite_acgih" class="form-label fw-semibold text-secondary small">Limite ACGIH</label>
                                <input type="text" name="limite_acgih" id="limite_acgih" class="form-control border-dark-subtle" value="<?= htmlspecialchars($risco['limite_acgih'] ?? '') ?>">
                            </div>

                            <div class="col-12 col-md-4">
                                <label for="nivel_acao" class="form-label fw-semibold text-secondary small">Nível de Ação</label>
                                <input type="text" name="nivel_acao" id="nivel_acao" class="form-control border-dark-subtle" value="<?= htmlspecialchars($risco['nivel_acao'] ?? '') ?>">
                            </div>

                            <div class="col-12 col-md-6">
                                <label for="normas_aplicaveis" class="form-label fw-semibold text-secondary small">Normas Aplicáveis</label>
                                <textarea name="normas_aplicaveis" id="normas_aplicaveis" rows="3" class="form-control border-dark-subtle"><?= htmlspecialchars($risco['normas_aplicaveis'] ?? '') ?></textarea>
                            </div>

                            <div class="col-12 col-md-6">
                                <label for="metodologia" class="form-label fw-semibold text-secondary small">Metodologia</label>
                                <textarea name="metodologia" id="metodologia" rows="3" class="form-control border-dark-subtle"><?= htmlspecialchars($risco['metodologia'] ?? '') ?></textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row g-3">
                            <div class="col-12 col-md-4">
                                ...
                                <select name="severidade_padrao" id="severidade_padrao" class="form-select border-dark-subtle">
                                    <?php for ($i = 1; $i <= 5; $i++): ?>
                                        <option value="<?= $i ?>" <?= (int)($risco['severidade_padrao'] ?? 1) === $i ? 'selected' : '' ?>>
                                            <?= $i ?>
                                        </option>
                                    <?php endfor; ?>
                                </select>
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <select name="probabilidade_padrao" id="probabilidade_padrao" class="form-select border-dark-subtle">
                                    <?php for ($i = 1; $i <= 5; $i++): ?>
                                        <option value="<?= $i ?>" <?= (int)($risco['probabilidade_padrao'] ?? 1) === $i ? 'selected' : '' ?>>
                                            <?= $i ?>
                                        </option>
                                    <?php endfor; ?>
                                </select>
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <div class="form-check form-switch d-flex align-items-center gap-3 ps-0 mt-2">
                                    <input class="form-check-input m-0 border-dark-subtle" type="checkbox" id="ativo" name="ativo" value="1" <?= !empty($risco['ativo']) ? 'checked' : '' ?>>
                                    <div>
                    ...
                    <div class="d-flex justify-content-end gap-2 border-top pt-3">
                        <a href="<?= BASE_URL ?>/riscos/<?= htmlspecialchars($slug ?? '') ?>" class="btn btn-outline-danger rounded-pill px-4 fw-medium">Cancelar</a>
                        <button type="submit" class="btn btn-success rounded-pill px-4 fw-medium shadow-sm">
                            <i class="fas fa-check me-1"></i> Atualizar Risco
                        </button>
                    </div>
