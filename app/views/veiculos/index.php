...
<main class="content flex-grow-1 pt-3 px-4 pb-4 bg-light-subtle">
    <div class="container-fluid px-2 px-lg-4 mb-4">
        ...
        <header class="page-header-veiculos mb-4 px-4 py-3 bg-white border rounded-3 shadow-sm d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div>
                <h3 class="m-0 fw-bold text-dark d-flex align-items-center gap-3" style="font-size: 1.5rem;">
                    <span class="icon-container d-flex align-items-center justify-content-center"
                          style="width: 38px; height: 38px; background: linear-gradient(135deg, #0d6efd, #084298); border-radius: 8px;">
                ...
                <small class="text-muted d-block mt-1">
                    Gestão da frota disponível para visitas técnicas, inspeções e atendimentos operacionais.
                </small>
            </div>
        ...
        <?php if (!empty($veiculos)): ?>
            ...
            <section class="veiculos-app-view">

                <div class="veiculos-app-toolbar bg-white border rounded-4 shadow-sm p-3 mb-3">
                    <div class="input-group veiculos-app-search">
                        ...
                        <input
                            ...
                            placeholder="Buscar veículo, placa, cor ou status...">
                    </div>

                    <div class="d-flex align-items-center justify-content-between mt-3 gap-2">
                        <small class="text-muted">
                            <span id="contadorVeiculos"><?= count($veiculos ?? []) ?></span>
                            veículo(s) exibido(s)
                        </small>
                    </div>
                </div>

                <div class="veiculos-app-list">
                    <?php foreach ($veiculos as $vei): ?>
                        <?php
                        $veiculoAtivo = !empty($vei['ativo']);
                        $placa = htmlspecialchars($vei['placa'] ?? '-');
                        $placaFormatada = strlen($placa) === 7 ? substr($placa, 0, 3) . '-' . substr($placa, 3) : $placa;

                        $textoBusca = strtolower(
                            ($vei['modelo'] ?? '') . ' ' .
                            ($vei['placa'] ?? '') . ' ' .
                            ($vei['cor'] ?? '') . ' ' .
                            ($veiculoAtivo ? 'disponivel ativo' : 'inativo manutencao')
                        );
                        ?>

                        <article class="veiculo-app-card" data-search="<?= htmlspecialchars($textoBusca) ?>">

                            <div class="veiculo-app-card-main">
                                <div class="veiculo-app-icon">
                                    <i class="fas fa-car"></i>
                                </div>

                                <div class="veiculo-app-content">
                                    <h5><?= htmlspecialchars($vei['modelo'] ?? 'Veículo sem modelo') ?></h5>

                                    <p class="veiculo-app-placa">
                                        <?= htmlspecialchars($placaFormatada) ?>
                                    </p>

                                    <div class="veiculo-app-meta">
                                        <span>
                                        ...
                                        <span>
                                            ...
                                            <?= !empty($vei['cor']) ? htmlspecialchars($vei['cor']) : 'Cor não informada' ?>
                                        </span>

                                        <span>
                                            <i class="<?= $veiculoAtivo ? 'fas fa-circle-check text-success' : 'fas fa-circle-xmark text-danger' ?>"></i>
                                            <?= $veiculoAtivo ? 'Disponível' : 'Inativo / Manutenção' ?>
                                        </span>
                                    </div>
                                </div>
                            </div>

                            <div class="veiculo-app-actions">
                                <button
                                ...
                                <a href="<?= BASE_URL ?>/veiculos/editar/<?= $vei['id'] ?>" class="btn btn-outline-primary rounded-pill px-3">
                ...
                <div id="semResultadoVeiculos" class="text-center py-5 text-muted d-none">
                    <i class="fas fa-magnifying-glass fa-2x mb-3 opacity-50"></i>
                    <h6 class="fw-bold text-dark">Nenhum veículo encontrado</h6>
                    <p class="small mb-0">Tente buscar por modelo, placa, cor ou status.</p>
                </div>
            </section>
                ...
                <div class="card-body p-4">
                    <div class="text-center py-5 text-muted">
                        ...
                        <p class="small text-muted mb-3">
                            Clique no botão abaixo para adicionar o primeiro veículo à frota.
                        </p>
                ...
                <div class="modal-content border-0 shadow-lg rounded-3">
                    ...
                    <div class="modal-header bg-light border-bottom py-3">
                        <h5 class="modal-title fw-bold text-dark d-flex align-items-center gap-2">
                            <i class="fas fa-car border p-2 bg-light rounded-3 text-secondary"></i>
                            Ficha do Veículo #<?= $vei['id'] ?>
                        </h5>
                    ...
                    <div class="modal-body p-4">
                        ...
                        <div class="mb-3 border-bottom pb-3">
                            <label class="text-secondary small fw-semibold d-block mb-1">
                                Status Operacional
                            </label>
                        ...
                        <div class="text-end">
                            <small class="text-muted d-block fs-7">
                                ...
                                Cadastrado em:
                                <?= !empty($vei['criado_em']) ? date('d/m/Y H:i', strtotime($vei['criado_em'])) : 'Data não informada' ?>
                            </small>
                        </div>
                    </div>

                    <div class="modal-footer bg-light py-3">
                        <a href="<?= BASE_URL ?>/veiculos/editar/<?= $vei['id'] ?>" class="btn btn-primary rounded-pill px-4">
                            <i class="fas fa-edit me-1"></i>
                            Editar
                        </a>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const input = document.getElementById('buscarVeiculo');
    const cards = document.querySelectorAll('.veiculo-app-card');
    const contador = document.getElementById('contadorVeiculos');
    const semResultado = document.getElementById('semResultadoVeiculos');
